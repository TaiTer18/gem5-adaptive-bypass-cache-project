...
if __name__ == "__main__":
    ...
    # Point directly to the specific bt.S.x benchmark the user compiled
    bt_bin_path = os.path.join(gem5_root, 'NPB3.3.1/NPB3.3-SER/bin/bt.S.x')
    
    if os.path.isfile(bt_bin_path) and os.access(bt_bin_path, os.X_OK):
        bm_list.append(('bt.S', bt_bin_path))
    else:
        print(f"Error: Could not find executable {bt_bin_path}")
        print("Please ensure you compiled it successfully.")
        exit(1)
            ...
            for assoc in l2_assocs:
                for policy in policies:
                    ...
                    for prob in probs_to_test:
                        ...
                        outdir = f'results/X86/bt_eval/{bm_name}/{l2_size}/{assoc}w/{policy}/{prob}'
