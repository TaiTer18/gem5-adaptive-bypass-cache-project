...
def main():
    print("Extracting O3CPU verification stats...")
    base_dir = "results/X86_O3/verification/cg.S/1MB/8w"
    ...
    plt.title(f'Out-of-Order IPC Verification (CG Benchmark, 1MB L2)\nSpeedup: {speedup:.4f}x (+{improvement_pct:.2f}%)')
    plt.ylabel('Instructions Per Cycle (IPC)')
    
    out_dir = "plots_all"
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, '7_o3_ipc_verification.png')
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\nSuccessfully generated Out-of-Order verification graph: {out_path}")
