...
if __name__ == "__main__":
    ...
    # Point directly to the specific lu.S.x benchmark
    lu_bin_path = os.path.join(gem5_root, 'NPB3.3.1/NPB3.3-SER/bin/lu.S.x')
    
    if os.path.isfile(lu_bin_path) and os.access(lu_bin_path, os.X_OK):
        bm_list.append(('lu.S', lu_bin_path))
    else:
        print(f"Error: Could not find executable {lu_bin_path}")
        exit(1)
            ...
            for assoc in l2_assocs:
                for policy in policies:
                    ...
                    for prob in probs_to_test:
                        ...
                        outdir = f'results/X86/lu_eval/{bm_name}/{l2_size}/{assoc}w/{policy}/{prob}'
